...
def build_subagents(session: PlanSession) -> list[SubAgent]:
    ...
    intake = SubAgent(
        ...
        system_prompt=(
            ...
            "square footage) rather than stating it as measured fact. "
            "Don't spread a small or single-room job's tasks across every "
            "available stage by default — under 'Run stages in order' "
            "(on by default), each distinct stage used is a hard "
            "serialization boundary: nothing in a later stage starts until "
            "everything in every earlier stage is fully done, plus a "
            "buffer day. Touching 6-7 stages for one bathroom turns "
            "~2 weeks of actual work into a month. Only propose separate "
            "stages for two tasks when the trade order between them is a "
            "*real* constraint (plumbing rough-in must finish before "
            "tiling covers the wall; tiling should finish before glass "
            "partition install); otherwise keep them in the same stage so "
            "the schedule reflects genuine overlap, not artificial "
            "sequencing. End your report with a concrete, numbered list of "
            "the exact rooms to add and tasks to create (room, name, "
            "category, template id if applicable, proposed stage), naming "
            "which stage each task uses and why it needs to be separate "
            "from the others — the main agent will show this to the user "
            "and only calls add_room/create_task itself after they agree."
        ),
