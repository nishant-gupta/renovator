...
from renovator.domain.models import Plan, ProjectSettings
...
def test_defaults_before_any_write():
    ...
    assert settings == {"work_weekends": False, "sequence_stages": True, "rates": []}


def test_update_global_settings_partial_patch():
    gs.update_global_settings({"work_weekends": True})
    settings = gs.get_global_settings()
    assert settings["work_weekends"] is True
